...
import { accessGranted, changeLevel, hitui, listen, yeah } from "src/effects/effects";
...
export class Level1 {
  ...
  start() {
    ...
    let textA = new Entity();
    textA.addComponent(new TextShape(getQuestionA(0)));
    textA.getComponent(TextShape).fontSize = 2;
    textA.getComponent(TextShape).shadowColor = Color3.Gray();
    textA.getComponent(TextShape).paddingBottom = 9;
    textA.getComponent(TextShape).shadowOffsetY = 1;
    textA.getComponent(TextShape).shadowOffsetX = -1;
    engine.addEntity(textA);
    textA.setParent(doorask1);

    let textB = new Entity();
    textB.addComponent(new TextShape(getQuestionB(0)));
    textB.getComponent(TextShape).shadowColor = Color3.Gray();
    textB.getComponent(TextShape).fontSize = 2;
    textB.getComponent(TextShape).paddingBottom = 9;
    textB.getComponent(TextShape).shadowOffsetX = 1;
    textB.getComponent(TextShape).shadowOffsetX = -1;
    engine.addEntity(textB);

    textB.setParent(doorask2);
    let textC = new Entity();
    textC.addComponent(new TextShape(getQuestionC(0)));
    textC.getComponent(TextShape).shadowColor = Color3.Gray();
    textC.getComponent(TextShape).fontSize = 2;
    textC.getComponent(TextShape).paddingBottom = 9;
    textC.getComponent(TextShape).shadowOffsetY = 1;
    textC.getComponent(TextShape).shadowOffsetX = -1;
    engine.addEntity(textC);
    textC.setParent(doorask3);
